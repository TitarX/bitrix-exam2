...
// ex2-83 >>>
$arNavigation = false;
if (is_numeric($arParams['CLASSIF_ELEMENTS_PER_PAGE'])) {
	$arNavigation = CDBResult::GetNavParams($arParams['CLASSIF_ELEMENTS_PER_PAGE']);
}
// <<< ex2-83

if ($this->StartResultCache(false, array($arNavigation))) { // ex2-83
	$arItemsResult = array();
	...
	$rsSection = CIBlockSection::GetList(
		...
		array('nPageSize' => $arParams['CLASSIF_ELEMENTS_PER_PAGE'])
	);
	// ex2-83 >>>
	$rsSection->NavStart($arParams['CLASSIF_ELEMENTS_PER_PAGE']);
	$rsSection->NavPrint($arParams['CLASSIF_ELEMENTS_PAGES_NAME']);
	// <<< ex2-83
	while ($arSection = $rsSection->Fetch()) {
	...
	$rsElement = CIBlockElement::GetList(
		...
		false,
		false,
		array('IBLOCK_ID', 'IBLOCK_SECTION_ID', 'ID', 'NAME', 'PROPERTY_PRICE', 'PROPERTY_MATERIAL', 'PROPERTY_ARTNUMBER', 'DETAIL_PAGE_URL')
	);
